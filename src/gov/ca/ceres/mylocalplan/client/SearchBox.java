...
import edu.ucdavis.cstars.client.tasks.GeneralizeParameters;
...
public class SearchBox extends TextBox {
    ...
    private static LinkedList<LocalPlanQueryTask> queries = new LinkedList<LocalPlanQueryTask>();
    ...
    public SearchBox() {
        ...
        JsArray<ServiceConfig> searchServices = ((MyLocalPlanConfig) AppManager.INSTANCE.getConfig()).getSearchServices();
        for( int i = 0; i < searchServices.length(); i++ ) {
            if( searchServices.get(i).getType().equals("geocoder") ) {
                ...
                LocalPlanQueryTask cqt = (LocalPlanQueryTask) QueryTask.create(searchServices.get(i).getUrl());
    ...
    public void setMap(MapWidget map) {
        ...
        FeatureCollectionDataLayer layer = new FeatureCollectionDataLayer("Standard Features");
    ...
    private void getGeometry(final SearchResult sr) {
        LocalPlanQueryTask qt = null;
        for( LocalPlanQueryTask cqt: queries ) {
    ...
    private static class QueryAll {
        ...
        public QueryAll(String searchTxt, QueryComplete callback) {
            ...
            for( LocalPlanQueryTask qt: queries ) query(qt);
        }
        ...
        private void query(final LocalPlanQueryTask qt) {
    ...
    public static class LocalPlanQueryTask extends QueryTask {
        
        protected LocalPlanQueryTask() {}
